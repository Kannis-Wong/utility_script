#Invoke-ps2exe -inputFile .\IPv4NetworkScanFunc.ps1 -outputFile SBXScanningTool.exe -x64 -iconFile .\favicon.ico -version '1.0.0' -company 'SandboxVR' -noConfigFile -requireAdmin
...
function customizeIPv4Scan{
    Remove-Variable -Name startIP -ErrorAction SilentlyContinue
    Remove-Variable -Name endIP -ErrorAction SilentlyContinue
    Write-Host "Option: [3]`nYou choose to customize IP checking...`nHint:`nEnter Start IPv4-Address like 10.1.2.1`nEnter End IPv4-Address like 10.1.2.255`nIt will scan the device status between 10.1.2.1 to 10.1.2.255" -BackgroundColor Black -ForegroundColor Cyan
    Do{
        try{
            $numOk = $true
            $startIP = Read-Host "Start IPv4-Address"
            $endIP = Read-Host "End IPv4-Address"
        }
        catch{
            $numOK = $false
            Write-Host "String is invalid...`n" -ForegroundColor Red -BackgroundColor Black
        }
    }until($numOk)
    $List = @()
    $temp = @()
    try{    
        $temp += IPv4NetworkScan -StartIPv4Address $startIP -EndIPv4Address $endIP
    }
    catch{
        "An error occurred that could not be resolved. Have you entered invalid IP address like 10.1.1.1.? Please verify your input.`n"
    }
    #Write-Host "Please Be Patient... the tool is running..." -ForegroundColor Green
    foreach($item in $temp){
        $List += [PSCustomObject]@{
            IPAddress = $item.IPv4Address.IPAddressToString
            Status = $item.Status
            Hostname = $item.Hostname
        }
    }
    if($List){
        $List | Out-GridView -Title 'Device online status'
    }
    else{
        Write-Host "No device is online...`n" -ForegroundColor Cyan
    }
}
...
function scanning{
    ...
    Write-Host "Please Be Patient... the tool is running...`n" -ForegroundColor Green
    $List = @()
    $temp = @()
    foreach($ip in $targetList){
        $temp += IPv4NetworkScan -IPv4Address $ip -CIDR 24
    }
    foreach($item in $temp){
        $List += [PSCustomObject]@{
            IPAddress = $item.IPv4Address.IPAddressToString
    ...
    if($List){
        ...
        Write-Host "No device is online...`n" -ForegroundColor Yellow
    }
}

function scanningSubNet{
    Write-Host "Option: [4]`nYou choose to customize IP subnet checking...`nHint:`nEnter IPv4-Address which is in the subnet like 10.1.2.0`nIt will scan the device status in the subnet 10.1.2.0-255." -BackgroundColor Black -ForegroundColor Magenta
    Do{
        try{
            $numOk = $true
            $IPsubnet = Read-Host "IPv4-Address which is in the subnet"
        }
        catch{
            $numOK = $false
            Write-Host "String is invalid..." -ForegroundColor Red -BackgroundColor Black
        }
    }until($numOk)
    $List = @()
    $temp = @()
    try{    
        $temp += IPv4NetworkScan -IPv4Address $IPsubnet -CIDR 24
    }
    catch{
        "An error occurred that could not be resolved. Have you entered invalid IP address subnet like 10.1.1.0.? Please verify your input.`n"
    }
    foreach($item in $temp){
        $List += [PSCustomObject]@{
            IPAddress = $item.IPv4Address.IPAddressToString
    ...
    if($List){
        ...
        Write-Host "No device is online...`n" -ForegroundColor Magenta
    }
...
while(1){
    $serverlist = @('10.1.1.0','10.2.1.0','10.3.1.0','10.4.1.0','10.5.1.0','10.6.1.0')
    $bplist = @('10.1.2.0','10.2.2.0','10.3.2.0','10.4.2.0','10.5.2.0','10.6.2.0')
    $temp = @()
    Do{
        try{
            $numOk = $true
            Write-Host "[1]" -BackgroundColor Black -ForegroundColor Yellow -NoNewline; Write-Host " Check Server Status...| room: 1-6 | " ;
            Write-Host "[2]" -BackgroundColor Black -ForegroundColor Green -NoNewline; Write-Host " Check Backpacks Status...| room: 1-6 | " ;
            Write-Host "[3]" -BackgroundColor Black -ForegroundColor Cyan -NoNewline; Write-Host " Customize IP range that want to check..." ;
            Write-Host "[4]" -BackgroundColor Black -ForegroundColo Magenta -NoNewline; Write-Host " Customize IP subnet that want to check..." ;
            Write-Host "[5]" -BackgroundColor Black -ForegroundColo Red -NoNewline; Write-Host " Quit the app..." ;
            [int]$roomNumber = Read-Host "Enter number: "
            if(($roomNumber -gt 5) -or ($roomNumber -lt 1)){
                Write-Host "Please enter the number between 1-4 ...`n" -ForegroundColor Red -BackgroundColor Black
            }
        }
        catch{
            $numOK = $false
            Write-Host "Please enter the number between 1-5 ... string is invalid...`n" -ForegroundColor Red -BackgroundColor Black
        }
    }until((($roomNumber -ge 1) -and ($roomNumber -le 5)) -and $numOk)
    switch($roomNumber){
        1{Write-Host "Checking Server Status..."; $targetList = $serverlist; scanning -tarList $targetList}
        2{Write-Host "Checking Backpacks Status..."; $targetList = $bplist; scanning -tarList $targetList}
        3{Write-Host "Customize IP range..."; customizeIPv4Scan }
        4{Write-Host "Customize IP subnet..."; scanningSubNet }
        5{exit}
    }
}
